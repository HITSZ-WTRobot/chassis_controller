...
void Omni4_Init(Omni4_t* chassis, const Omni4_Config_t* config) {
    ...
    float half_x = config->wheel_distance_x * 1e-3f * 0.5f;
    float half_y = config->wheel_distance_y * 1e-3f * 0.5f;
...
void Omni4_ApplyVelocity(Omni4_t* chassis, const float vx, const float vy, const float wz) {
    // 转换角速度单位：deg/s -> rad/s
    float wz_rad = DEG2RAD(wz);
    
    // 四个轮子的线速度计算（全向轮核心公式）
    float v_fr =  vx * cos(chassis->angle) + vy * sin(chassis->angle) + chassis->half_diag * wz_rad; // 右前轮
    float v_fl =  -vx * cos(chassis->angle) + vy * sin(chassis->angle) + chassis->half_diag * wz_rad; // 左前轮
    float v_rl =  -vx * cos(chassis->angle) - vy * sin(chassis->angle) + chassis->half_diag * wz_rad; // 左后轮
    float v_rr =  vx * cos(chassis->angle) - vy * sin(chassis->angle) + chassis->half_diag * wz_rad; // 右后轮
...
float Omni4Forward_GetYaw(const Omni4_t* chassis) {
    float angle_fr = Motor_GetAngle(chassis->wheel[OMNI4_WHEEL_FR]->motor_type, 
                                   chassis->wheel[OMNI4_WHEEL_FR]->motor);
    float angle_fl = Motor_GetAngle(chassis->wheel[OMNI4_WHEEL_FL]->motor_type, 
                                   chassis->wheel[OMNI4_WHEEL_FL]->motor);
    float angle_rl = Motor_GetAngle(chassis->wheel[OMNI4_WHEEL_RL]->motor_type, 
                                   chassis->wheel[OMNI4_WHEEL_RL]->motor);
    float angle_rr = Motor_GetAngle(chassis->wheel[OMNI4_WHEEL_RR]->motor_type, 
...
float Omni4Forward_GetX(const Omni4_t* chassis) {
    ...
    return (chassis->wheel_radius / 4) * 
           DEG2RAD(angle_fr / cos(chassis->angle)
                   - angle_fl / cos(chassis->angle) 
                   - angle_rl / cos(chassis->angle) 
                   + angle_rr / cos(chassis->angle));
}
...
float Omni4Forward_GetY(const Omni4_t* chassis) {
    ...
    return (chassis->wheel_radius / 4) * 
           DEG2RAD(angle_fr / sin(chassis->angle)
                   + angle_fl / sin(chassis->angle)
                   - angle_rl / sin(chassis->angle)
                   - angle_rr / sin(chassis->angle));
}